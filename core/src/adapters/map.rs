    ...
    I: IntoIterator<Item = (L, U)>,
{
    fn serialize(self, serializer: Serializer) -> Result<(), SerializeError> {
        serializer.serialize_map2_iter(self.0)
    }
}
...
#[derive(Debug)]
pub(crate) struct IterAsMap1<T>(pub T);

impl<K, T, I> PrimaryTag for IterAsMap1<I>
where
    K: PrimaryKeyTag,
    T: PrimaryTag,
    I: IntoIterator<Item = (K, T)>,
...
impl<K, L, T, U, I> Serialize<Map<K, T>> for IterAsMap1<I>
where
    K: KeyTag,
    L: SerializeKey<K>,
    T: Tag,
...
{
    fn serialize(self, serializer: Serializer) -> Result<(), SerializeError> {
        serializer.serialize_map1_iter(self.0)
    }
}

impl<'a, K, L, T, U, I> Serialize<Map<K, T>> for &'a IterAsMap1<I>
where
    K: KeyTag,
    L: SerializeKey<K>,
    T: Tag,
