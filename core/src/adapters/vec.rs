    ...
    U::Item: Serialize<T>,
{
    fn serialize(self, serializer: Serializer) -> Result<(), SerializeError> {
        serializer.serialize_vec2_iter(self.0)
    }
}
...
#[derive(Debug)]
pub(crate) struct IterAsVec1<T>(pub T);

impl<T> PrimaryTag for IterAsVec1<T>
where
    T: IntoIterator,
    T::Item: PrimaryTag,
{
...
impl<T, U> Serialize<tags::Vec<T>> for IterAsVec1<U>
where
    T: Tag,
    U: IntoIterator,
...
{
    fn serialize(self, serializer: Serializer) -> Result<(), SerializeError> {
        serializer.serialize_vec1_iter(self.0)
    }
}

impl<'a, T, U> Serialize<tags::Vec<T>> for &'a IterAsVec1<U>
where
    T: Tag,
    &'a U: IntoIterator,
