...
impl Server {
    ...
    fn add(&mut self, call: Call<Bookmark, (), BookmarkError>) -> Result<()> {
        ...
        // bookmark.icon.as_ref().map(String::is_empty);
...
async fn add(args: Add, bus: &Handle) -> Result<()> {
    ...
    if args.icon.is_some() && (bookmarks.version() < 2) && !args.ignore.ignore_version {
        return Err(anyhow!("icons are not supported version 1 servers"));
    }
...
async fn get(server: Option<ObjectUuid>, bus: &Handle) -> Result<()> {
    ...
    if list.is_empty() {
        ...
        for bookmark in list {
            if let Some(icon) = bookmark.icon {
                println!("  `{}`: {} (icon `{icon}`)", bookmark.name, bookmark.url);
            } else {
