use crate::bookmarks_v2::{Bookmarks as Bookmarks2, BookmarksCall as Bookmarks2Call};
use crate::bookmarks_v3::{
    Bookmark, Bookmarks3, Bookmarks3Call, Bookmarks3Event, Bookmarks3Proxy, Error as BookmarkError,
};
use crate::{IgnoreVersionArg, ServerArg};
...
pub struct Add {
    ...
    /// The icon of the bookmark to add.
    #[clap(long)]
    icon: Option<String>,
...
impl Server {
    async fn new(bus: &Handle) -> Result<Self> {
        ...
        let svc_v2 = Bookmarks2::new(&obj).await?;
        let svc_v3 = Bookmarks3::new(&obj).await?;
    ...
    async fn run(mut self) -> Result<()> {
        loop {
            tokio::select! {
                call = self.svc_v2.next_call() => {
                    match call {
                        Some(Ok(call)) => self.handle_call2(call)?,
                        Some(Err(e)) => self.invalid_call(e),
                        None => break Err(anyhow!("broker shut down")),
                    }
                }

                Some(call) = self.svc_v3.next_call() => {
                    match call {
                        Ok(call) => self.handle_call3(call)?,
                        Err(e) => self.invalid_call(e),
                    }
                }
    ...
    fn handle_call3(&mut self, call: Bookmarks3Call) -> Result<()> {
        match call {
            // BookmarksCall::Get(call) => self.get(call)?,
            // BookmarksCall::Add(call) => self.add(call)?,
            // BookmarksCall::Remove(call) => self.remove(call)?,
            // BookmarksCall::GetGroups(call) => self.get_groups(call)?,
            Bookmarks3Call::UnknownFunction(call) => self.unknown_function(call),
            _ => todo!(),
        }
...
impl fmt::Display for BookmarkError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::InvalidIcon => write!(f, "invalid icon"),
