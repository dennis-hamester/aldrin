...
fn enum_fallback_new_to_old() {
    ...
    let Old::Unknown(fallback) = old else {
        panic!()
    };

    assert_eq!(fallback.variant(), 2);
    assert_eq!(fallback.deserialize(), Ok(1u32));

    let serialized = SerializedValue::serialize(&Old::Unknown(fallback)).unwrap();
