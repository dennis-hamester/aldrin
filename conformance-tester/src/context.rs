...
impl Context {
    ...
    pub(crate) fn remove_client(&mut self, id: &ClientId) -> Result<()> {
        self.clients
            .remove(id)
            .map(|_| ())
    ...
    pub(crate) fn get_client(&mut self, id: &ClientId) -> Result<&Client> {
        self.clients
            .get(id)
            .ok_or_else(|| anyhow!("unknown client `{id}`"))
    }

    pub(crate) fn get_client_mut(&mut self, id: &ClientId) -> Result<&mut Client> {
        self.clients
            .get_mut(id)
            .ok_or_else(|| anyhow!("unknown client `{id}`"))
    }

    pub(crate) fn client_ids(&self) -> impl ExactSizeIterator<Item = &ClientId> {
        self.clients.keys()
    }

    pub(crate) fn get_serial(&self, id: &str) -> Result<u32> {
        self.serials
            .get(id)
            .copied()
    ...
    pub(crate) fn get_uuid(&self, id: &str) -> Result<Uuid> {
        self.uuids
            .get(id)
            .copied()
