    ...
    F: for<'a> RunHelper<'a>,
{
    ...
    let mut client = time::timeout_at(timeout, ClientUnderTest::new(args))
        .await
        .map_err(|_| RunError::bare(anyhow!("timeout while starting client")))??;

    let res = match time::timeout_at(timeout, test.call(&mut client)).await {
        Ok(Ok(())) => Ok(()),
        Ok(Err(e)) => Err(e),
