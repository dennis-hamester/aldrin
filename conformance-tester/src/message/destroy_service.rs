...
pub(crate) struct DestroyService {
    pub serial: Serial,
    pub cookie: UuidRef,
}

impl DestroyService {
    pub(crate) fn to_core(&self, ctx: &Context) -> Result<message::DestroyService> {
        let serial = self.serial.get(ctx)?;
        let cookie = self.cookie.get(ctx)?.into();

        Ok(message::DestroyService { serial, cookie })
    }
...
impl TryFrom<message::DestroyService> for DestroyService {
    type Error = Error;

    fn try_from(msg: message::DestroyService) -> Result<Self> {
        Ok(Self {
