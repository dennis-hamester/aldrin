...
impl SubscribeObjects {
    pub fn to_proto(&self, ctx: &Context) -> Result<message::SubscribeObjects> {
        let serial = self.serial.as_ref().map(|s| s.get(ctx)).transpose()?;

        Ok(message::SubscribeObjects { serial })
    }
...
impl TryFrom<message::SubscribeObjects> for SubscribeObjects {
    type Error = Error;

    fn try_from(msg: message::SubscribeObjects) -> Result<Self> {
        Ok(Self {
